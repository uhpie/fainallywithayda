      ...
      {TABS.map(({ id, label, Icon }) => {
        ...
        return (
          <button
            key={id}
            onClick={() => onChange(id)}
            className={`relative flex-1 flex flex-col items-center pt-2.5 pb-3 gap-1 transition-colors duration-200 ${
          ...
          >
            ...
            <Icon />
            <span className={`font-sans text-[9px] tracking-wider leading-none ${on ? 'font-semibold' : ''}`}>
              {label}
            </span>
          </button>
        )
